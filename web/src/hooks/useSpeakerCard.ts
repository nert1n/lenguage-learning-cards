...
const useSpeakerCard = ({
	...
	const delOne = useMutation(
		async ({ cardId, catId }: { cardId: number; catId: number }) =>
			CategoryService.deleteCategoryCard(jwt, catId, cardId),
		{
			onSuccess: () => {
	...
	const addCardToCategory = useMutation(
		({ cardId, catId }: { cardId: number; catId: number }) =>
			CategoryService.postCategoryAddCardExist(jwt, catId, cardId),
		{
			onSuccess: () => {
				queryClient.refetchQueries('edit');
				queryClient.refetchQueries('getCategoriesMain');
	...
	async function handleClickPlus() {
		try {
			if (path === '/dictionary') {
				await delFromDict.mutateAsync(id);
				setActive(false);
			}
