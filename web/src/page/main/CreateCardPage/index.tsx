...
function CreateCardPage({ catid }: { catid: number | undefined }) {
	...
	const error = () => {
		toast.info('Не все поля заполнены корректно');
	};
	...
	return (
		<form className={styles.wrapper} onSubmit={handleSubmit(onSubmit, error)}>
			...
			<MainInput
				...
				{...register('engtext', { required: true })}
			/>
			<MainInput
				type="text"
				placeholder="Русское слово"
				{...register('rustext', { required: true })}
			/>
